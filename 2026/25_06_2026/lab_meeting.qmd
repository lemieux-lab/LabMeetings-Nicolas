...
Tracking it down was a bit more involved than usual because the refactor was in progress and incomplete, so there were more places to look. The two culprits turned out to be straightforward once found, and the fixes are worth explaining.
...
For 100M k-mers, `seen` grows to 100M entries, roughly **4.5 GB** of temporary allocation.
...
The returned `Set` is the same in both versions: it only contains words actually shared. The saving is entirely in the intermediary: `seen` (100M entries, 4.5 GB) replaced by `ref_counts` (100M bytes, **100 MB**).
